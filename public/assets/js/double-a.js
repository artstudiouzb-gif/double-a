...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 9. Interactive Product Permit Checker
    const checkBtn = document.getElementById('checkBtn');
    const selectProduct = document.getElementById('product');
    const selectAction = document.getElementById('action');
    const resultDiv = document.getElementById('result');

    if (checkBtn && selectProduct && selectAction && resultDiv) {
        const checkerDb = {
            'agro_import': '<strong>Результат проверки:</strong><br><br>Для импорта СЗР и удобрений требуется государственная регистрация в Минсельхозе РУз. Обязательны полевые испытания (1 вегетационный период) и токсикологическая экспертиза.',
            'agro_production': '<strong>Результат проверки:</strong><br><br>Открытие химического производства в РУз подлежит лицензированию в сфере обращения сильнодействующих ядовитых веществ, разработке ТУ и обязательной экологической экспертизе (ЗВОС).',
            'agro_export': '<strong>Результат проверки:</strong><br><br>Для экспорта агрохимии требуется оформление фитосанитарного контроля Агентства карантина растений РУз и сертификата происхождения товара (СТ-1).',
            
            'food_import': '<strong>Результат проверки:</strong><br><br>Для ввоза продуктов питания и БАД требуется получение гигиенического заключения СЭС РУз, проведение лабораторных анализов на безопасность и регистрация декларации.',
            'food_production': '<strong>Результат проверки:</strong><br><br>Пищевые предприятия в РУз должны соответствовать принципам HACCP (ISO 22000). Продукция подлежит обязательной государственной регистрации в органах СЭС.',
            'food_export': '<strong>Результат проверки:</strong><br><br>Экспорт пищевой продукции в ЕС требует сертификации FSSC 22000, проверки упаковки на стандарты импортера и сертификата происхождения СТ-1.',
            
            'cosmetic_import': '<strong>Результат проверки:</strong><br><br>Поставка косметических товаров требует обязательного оформления санитарно-эпидемиологического заключения СЭС РУз и декларации о соответствии.',
            'cosmetic_production': '<strong>Результат проверки:</strong><br><br>Косметические фабрики должны функционировать на принципах GMP (ISO 22716). Каждая серия продукции подлежит лабораторному контролю.',
            'cosmetic_export': '<strong>Результат проверки:</strong><br><br>Экспорт парфюмерии и косметики требует оформления сертификата свободной продажи (Free Sale), сертификата происхождения и паспорта безопасности.',
            
            'vet_import': '<strong>Результат проверки:</strong><br><br>Импорт ветеринарных товаров требует регистрации препарата в Комитете ветеринарии РУз и ветеринарного разрешения на ввоз каждой партии.',
            'vet_production': '<strong>Результат проверки:</strong><br><br>Производство ветеринарных лекарств подлежит лицензированию ветеринарным Комитетом РУз и приведению цехов к стандартам ветеринарного GMP.',
            'vet_export': '<strong>Результат проверки:</strong><br><br>Экспортеру ветеринарных препаратов необходимо войти в Реестр экспортеров Комитета ветеринарии РУз и оформить международный ветеринарный сертификат.'
        };
        ...
        checkBtn.addEventListener('click', () => {
            ...
            if (!product || !action) {
                resultDiv.innerHTML = '<span style="color:#d43f3f">Пожалуйста, выберите оба параметра.</span>';
                resultDiv.classList.add('show');
                return;
            }
            const key = `${product}_${action}`;
            const text = checkerDb[key] || 'Информация по вашему запросу уточняется. Пожалуйста, свяжитесь с нашим экспертом.';
